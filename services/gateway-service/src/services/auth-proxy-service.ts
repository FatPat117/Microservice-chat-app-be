...
const resolvedMessage = (status:number,data:unknown):string =>{
  ...
  return status >= 500 ? "Authentication service is not available" : "An error occurred while processing your request";
}


const handleAxiosError = (error:unknown):never =>{
  if(!axios.isAxiosError(error) || !error.response){
    throw new HttpError(500,"Authentication service is not available");
  }
